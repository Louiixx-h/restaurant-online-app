package br.com.luishenrique.restaurantonline.data.repository

import android.util.Log
import androidx.lifecycle.MutableLiveData
import br.com.luishenrique.restaurantonline.data.model.*
import br.com.luishenrique.restaurantonline.data.network.RestaurantOnlineApi
import br.com.luishenrique.restaurantonline.utils.DisponibilityEnum
import kotlinx.coroutines.CoroutineScope
import kotlinx.coroutines.Dispatchers.IO
import kotlinx.coroutines.launch
import kotlinx.coroutines.withContext
import java.math.BigDecimal

class RestaurantOnlineRepository(private val apiService: RestaurantOnlineApi) {
    
    suspend fun findAllCategories(): List<Category> {
        var response:List<Category>?
        withContext(IO) {
            Log.i("luis", "findAllCategories: load data...")
            response = apiService.findAllCategories().body()
            Log.i("luis", "findAllCategories: loaded data")
        }
        return response ?: emptyList()
    }

    fun finAllCategoriesMock(): List<Category> {
        return listOf(
            Category("Fast Food", listOf(
                Food("Pizza",
                    "",
                    BigDecimal(27),
                    true,
                    DisponibilityEnum.ACTIVE,
                    listOf(Image(ImageFormat(
                        ImageType("https://img.itdg.com.br/tdg/images/blog/uploads/2019/05/pizza.jpg"),
                        ImageType("https://img.itdg.com.br/tdg/images/blog/uploads/2019/05/pizza.jpg"),
                        ImageType("https://img.itdg.com.br/tdg/images/blog/uploads/2019/05/pizza.jpg"),
                        ImageType("https://img.itdg.com.br/tdg/images/blog/uploads/2019/05/pizza.jpg")
                        ),
                        "https://img.itdg.com.br/tdg/images/blog/uploads/2019/05/pizza.jpg"
                        )
                    )
                ),
                Food("Hamburguer",
                    "",
                    BigDecimal(27),
                    true,
                    DisponibilityEnum.ACTIVE,
                    listOf(Image(ImageFormat(
                        ImageType("https://soubh.uai.com.br/uploads/post/image/11558/main_destaque_Alexandr_Popel.jpg"),
                        ImageType("https://soubh.uai.com.br/uploads/post/image/11558/main_destaque_Alexandr_Popel.jpg"),
                        ImageType("https://soubh.uai.com.br/uploads/post/image/11558/main_destaque_Alexandr_Popel.jpg"),
                        ImageType("https://soubh.uai.com.br/uploads/post/image/11558/main_destaque_Alexandr_Popel.jpg")
                    ),
                        "https://soubh.uai.com.br/uploads/post/image/11558/main_destaque_Alexandr_Popel.jpg"
                    )
                    )
                ),
                Food("Pastel",
                    "",
                    BigDecimal(27),
                    true,
                    DisponibilityEnum.ACTIVE,
                    listOf(Image(ImageFormat(
                        ImageType("https://xtudoreceitas.com/wp-content/uploads/Pastel-de-Carne-Temperado.jpg"),
                        ImageType("https://xtudoreceitas.com/wp-content/uploads/Pastel-de-Carne-Temperado.jpg"),
                        ImageType("https://xtudoreceitas.com/wp-content/uploads/Pastel-de-Carne-Temperado.jpg"),
                        ImageType("https://xtudoreceitas.com/wp-content/uploads/Pastel-de-Carne-Temperado.jpg")
                    ),
                        "https://xtudoreceitas.com/wp-content/uploads/Pastel-de-Carne-Temperado.jpg"
                    )
                    )
                )
            )),
            Category("Churrasco", listOf(
                Food("Coxão mole",
                    "",
                    BigDecimal(27),
                    true,
                    DisponibilityEnum.ACTIVE,
                    listOf(Image(ImageFormat(
                        ImageType("https://img.itdg.com.br/tdg/images/blog/uploads/2021/08/Churrasco-barato.jpg?mode=crop&width={:width=%3E150,%20:height=%3E130}"),
                        ImageType("https://img.itdg.com.br/tdg/images/blog/uploads/2021/08/Churrasco-barato.jpg?mode=crop&width={:width=%3E150,%20:height=%3E130}"),
                        ImageType("https://img.itdg.com.br/tdg/images/blog/uploads/2019/05/pizza.jpg"),
                        ImageType("https://img.itdg.com.br/tdg/images/blog/uploads/2019/05/pizza.jpg")
                    ),
                        "https://img.itdg.com.br/tdg/images/blog/uploads/2021/08/Churrasco-barato.jpg?mode=crop&width={:width=%3E150,%20:height=%3E130}"
                    )
                    )
                ),
                Food("Picanha",
                    "",
                    BigDecimal(27),
                    true,
                    DisponibilityEnum.ACTIVE,
                    listOf(Image(ImageFormat(
                        ImageType("https://mhmcdn.manualdohomemmoderno.com.br/?w=781&h=488&quality=100&clipping=crop&url=https://manualdohomemmoderno.com.br/files/2021/07/entenda-qual-e-a-ciencia-por-tras-do-churrasco-perfeito-entenda-qual-e-a-ciencia-por-tras-do-churrasco-perfeito.jpg"),
                        ImageType("https://mhmcdn.manualdohomemmoderno.com.br/?w=781&h=488&quality=100&clipping=crop&url=https://manualdohomemmoderno.com.br/files/2021/07/entenda-qual-e-a-ciencia-por-tras-do-churrasco-perfeito-entenda-qual-e-a-ciencia-por-tras-do-churrasco-perfeito.jpg"),
                        ImageType("https://mhmcdn.manualdohomemmoderno.com.br/?w=781&h=488&quality=100&clipping=crop&url=https://manualdohomemmoderno.com.br/files/2021/07/entenda-qual-e-a-ciencia-por-tras-do-churrasco-perfeito-entenda-qual-e-a-ciencia-por-tras-do-churrasco-perfeito.jpg"),
                        ImageType("https://mhmcdn.manualdohomemmoderno.com.br/?w=781&h=488&quality=100&clipping=crop&url=https://manualdohomemmoderno.com.br/files/2021/07/entenda-qual-e-a-ciencia-por-tras-do-churrasco-perfeito-entenda-qual-e-a-ciencia-por-tras-do-churrasco-perfeito.jpg")
                    ),
                        "https://mhmcdn.manualdohomemmoderno.com.br/?w=781&h=488&quality=100&clipping=crop&url=https://manualdohomemmoderno.com.br/files/2021/07/entenda-qual-e-a-ciencia-por-tras-do-churrasco-perfeito-entenda-qual-e-a-ciencia-por-tras-do-churrasco-perfeito.jpg"
                    )
                    )
                ),
                Food("Churrasco completo",
                    "",
                    BigDecimal(27),
                    true,
                    DisponibilityEnum.ACTIVE,
                    listOf(Image(ImageFormat(
                        ImageType("https://cptstatic.s3.amazonaws.com/imagens/enviadas/materias/materia7011/churrasco-cursos-cpt.jpg"),
                        ImageType("https://cptstatic.s3.amazonaws.com/imagens/enviadas/materias/materia7011/churrasco-cursos-cpt.jpg"),
                        ImageType("https://cptstatic.s3.amazonaws.com/imagens/enviadas/materias/materia7011/churrasco-cursos-cpt.jpg"),
                        ImageType("https://cptstatic.s3.amazonaws.com/imagens/enviadas/materias/materia7011/churrasco-cursos-cpt.jpg")
                    ),
                        "https://cptstatic.s3.amazonaws.com/imagens/enviadas/materias/materia7011/churrasco-cursos-cpt.jpg"
                    )
                    )
                )
            )),
            Category("Bebidas", listOf(
                Food("suco de laranja",
                    "",
                    BigDecimal(27),
                    true,
                    DisponibilityEnum.ACTIVE,
                    listOf(Image(ImageFormat(
                        ImageType("data:image/jpeg;base64,"),
                        ImageType("data:image/jpeg;base64,"),
                        ImageType("https://img.itdg.com.br/tdg/images/blog/uploads/2021/08/Churrasco-barato.jpg?mode=crop&width={:width=%3E150,%20:height=%3E130}"),
                        ImageType("https://img.itdg.com.br/tdg/images/blog/uploads/2021/08/Churrasco-barato.jpg?mode=crop&width={:width=%3E150,%20:height=%3E130}")
                    ),
                        "https://img.itdg.com.br/tdg/images/blog/uploads/2021/08/Churrasco-barato.jpg?mode=crop&width={:width=%3E150,%20:height=%3E130}"
                    )
                    )
                ),
                Food("Suco de angrião",
                    "",
                    BigDecimal(27),
                    true,
                    DisponibilityEnum.ACTIVE,
                    listOf(Image(ImageFormat(
                        ImageType("https://t2.rg.ltmcdn.com/pt/images/5/1/0/suco_de_agriao_8015_orig.jpg"),
                        ImageType("https://t2.rg.ltmcdn.com/pt/images/5/1/0/suco_de_agriao_8015_orig.jpg"),
                        ImageType("https://img.itdg.com.br/tdg/images/blog/uploads/2019/05/pizza.jpg"),
                        ImageType("https://img.itdg.com.br/tdg/images/blog/uploads/2019/05/pizza.jpg")
                    ),
                        "https://img.itdg.com.br/tdg/images/blog/uploads/2019/05/pizza.jpg"
                    )
                    )
                ),
                Food("Suco de morango",
                    "",
                    BigDecimal(27),
                    true,
                    DisponibilityEnum.ACTIVE,
                    listOf(Image(ImageFormat(
                        ImageType("https://1.bp.blogspot.com/-PLOd1ScdHe0/XPAzW2DVn4I/AAAAAAAAD2c/I5mFRrAuy7cBPtp0TZCe-ng10Ron-I_MwCLcBGAs/s1600/Suco%2Bde%2BMel%25C3%25A3o%252C%2BMorango%2Be%2BHortel%25C3%25A3.jpg"),
                        ImageType("https://1.bp.blogspot.com/-PLOd1ScdHe0/XPAzW2DVn4I/AAAAAAAAD2c/I5mFRrAuy7cBPtp0TZCe-ng10Ron-I_MwCLcBGAs/s1600/Suco%2Bde%2BMel%25C3%25A3o%252C%2BMorango%2Be%2BHortel%25C3%25A3.jpg"),
                        ImageType("https://img.itdg.com.br/tdg/images/blog/uploads/2019/05/pizza.jpg"),
                        ImageType("https://img.itdg.com.br/tdg/images/blog/uploads/2019/05/pizza.jpg")
                    ),
                        "https://img.itdg.com.br/tdg/images/blog/uploads/2019/05/pizza.jpg"
                    )
                    )
                )
            ))
        )
    }
}